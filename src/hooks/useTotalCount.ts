import { selectNumbers } from 'ducks/numbersSlice';
...
const useTotalCount = (): CountNumbers => {
  ...
  useEffect(() => {
      api('numbers').then((numbers: any[]) => {
        const countSetup = numbers.reduce(
          (accumulator: number, item: any) =>
            accumulator + parseFloat(item.setupPrice),
          0
        );
        const countMonthy = numbers.reduce(
          (accumulator: number, item: any) =>
            accumulator + parseFloat(item.monthyPrice),
          0
        );
